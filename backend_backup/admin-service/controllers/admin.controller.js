...
export const getDashboardData = async (req, res) => {
  try {
    ...
    const responseData = await fetchWithCache(cacheKey, 60, async () => {
      ...
      const errors = [];
      if (ordersResult.status === "rejected") errors.push("Order service is currently unavailable");
      if (usersResult.status === "rejected") errors.push("User service is currently unavailable");
