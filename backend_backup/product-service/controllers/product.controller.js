...
export const getProducts = async (req, res) => {
  try {
    ...
    res.json(products);
  } catch (err) { res.status(500).json({ message: "Failed to fetch products" }); }
};
...
export const createProduct = async (req, res) => {
  ...
  } catch (err) { res.status(500).json({ message: "Creation failed", error: err.message }); }
};
...
export const updateProduct = async (req, res) => {
  try {
    ...
    if (!product) return res.status(404).json({ message: "Product not found" });
    if (req.user.role === "vendor" && product.vendorId !== req.user.id) return res.status(403).json({ message: "Unauthorized" });
    ...
    if (stock !== undefined && stock >= 0) {
        ...
        if (product.availableStock > product.totalStock) product.availableStock = product.totalStock;
    }
...
export const getVendorInventory = async (req, res) => {
  try {
    const products = await Product.findAll({
      ...
      attributes: ['id', 'name', 'imageUrl', 'price', 'totalStock', 'reservedStock', 'warehouseStock', 'availableStock']
    });
...
export const getAllWarehouseInventory = async (req, res) => {
  try {
    const products = await Product.findAll({
      attributes: ['id', 'name', 'imageUrl', 'price', 'totalStock', 'reservedStock', 'warehouseStock', 'availableStock', 'vendorId'],
    ...
    res.json(formatted);
  } catch (err) { res.status(500).json({ message: "Failed to fetch admin inventory" }); }
};
...
export const transferToWarehouse = async (req, res) => {
  try {
    ...
    if((product.warehouseStock + quantity) > product.totalStock) {
        return res.status(400).json({message: `Cannot transfer. Warehouse stock cannot exceed Total stock (${product.totalStock}).`});
    }
...
export const updateWarehouseStock = async (req, res) => {
  try {
    ...
    if(warehouseStock > product.totalStock) return res.status(400).json({message: "Warehouse stock cannot exceed Total stock"});
